...
export default function NuevaVentaPage() {
  ...
  const agregarItem = (llanta: Llanta) => {
    ...
    setItems([
      ...items,
      {
        ...
        precioStr: llanta.precioVenta.toString(),
      },
    ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success && result.data) {
      router.push(`/inventario/ventas/${result.data.id}`);
    } else {
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (v === '' || /^\d+$/.test(v)) {
                                    const num = Math.min(parseInt(v) || 0, stockDisponible);
                                    actualizarItem(index, {
                                      cantidadStr: v === '' ? '' : String(num),
                                      cantidad: num,
                                    });
                                  }
                                }}
