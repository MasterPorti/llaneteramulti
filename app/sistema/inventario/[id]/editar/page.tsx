...
export default function EditarLlantaPage() {
  ...
  const loadLlanta = async () => {
    try {
      const response = await fetch(`/api/sistema/inventario/${id}`);
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      router.push(`/sistema/inventario/${id}`);
    } else {
      ...
      <div>
        ...
        <Card>
          <CardBody className="text-center py-12">
            ...
            <Link href="/sistema/inventario" className="btn btn-primary">
    ...
    <div>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: formData.medida, href: `/sistema/inventario/${id}` },
          { label: 'Editar' },
        ]}
      />
        ...
        <CardBody>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-[var(--border-color)]">
              <Link href={`/sistema/inventario/${id}`} className="btn btn-secondary">
                Cancelar
              </Link>
              <button
                type="submit"
